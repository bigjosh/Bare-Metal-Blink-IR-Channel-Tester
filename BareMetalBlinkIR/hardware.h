...
#define BUTTON_PCI_BIT     PCIE2        // Pin change interrupt enable bit PCICR
#define BUTTON_IFR_BIT     PCIF2        // Pin change interrupt flag bit in PCIFR
#define BUTTON_ISR         PCINT2_vect

#define BUTTON_MASK        PCMSK2
#define BUTTON_PCINT_BIT   PCINT23
...
// GPIOR register allocations
// These registers are super handy, but they are global so we much share nicely
// When adding a new one makes sure not already used for an existing one

// Used for IR TX to signal to the ISR that the foreground did load a new value
// into the timer

#define IR_TX_SEMAPHONE_REG         GPIOR1
#define IR_TX_SEMAPHORE_BIT              0
